...
public class GrenadierKit extends Kit{
	...
	public static void handleClick(Player player){
		...
		egg.setVelocity(player.getLocation().getDirection().multiply(grenadierSpeed));
	...
	public static void handleStrike(Entity entity, Player shooter, Player hitPlayer, Main main){
		if(hitPlayer != null){
			DeathManager.handleDeath(hitPlayer, shooter, Arena.getGameMode(), KitType.GRENADIER.name(), true, main);
		}
		List<Entity> entities = entity.getNearbyEntities(grenadierRadius, grenadierRadius, grenadierRadius);
				...
				if(e.getType() == EntityType.PLAYER){
					...
					if(Utils.getBlue().contains(p.getUniqueId())){
						if(ProjectileHitListener.hitIsValid(p, shooter, null)){
							DeathManager.damage(p, shooter, KitType.GRENADIER.name(), main, false);	
						}
					}
				}
			}
		...
		ExplosionHandler.createExplosion(shooter, grenadierBallCount, entity.getLocation(), grenadierBallSpeed, KitType.GRENADIER.name(), main);
	}
}
